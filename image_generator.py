...
Orijinal görsel varsa kullanır, yoksa siyah-kırmızı şablon yapar.
"""
...
def create_image(news_item: dict) -> Path:
    ...
    if base:
        # Orijinal görseli kare yap + koyulaştır
        w, h = base.size
        s = min(w, h)
        ...
        # Siyah-kırmızı şablon
        img = Image.new("RGB", SIZE, (10, 10, 10))
        draw = ImageDraw.Draw(img)
        for i in range(6):          # kırmızı yatay çizgiler
            y = 80 + i * 160
    ...
    # Kategori etiketi (üst sol)
    category = news_item.get("category", "general")
    label = CATEGORY_LABELS.get(category, "⚡ METAL")
    ...
    for line in lines:
        draw.text((52, y+2), line, font=f_title, fill=(0, 0, 0))   # gölge
        draw.text((50, y),   line, font=f_title, fill=(240, 240, 240))
        y += 68

    # Kaynak (en alt)
    f_src = _font(28)
    draw.text((50, SIZE[1]-55), f"📰 {news_item['source']}", font=f_src, fill=(160, 160, 160))

    # Hesap adı (sağ alt)
    handle = os.getenv("IG_HANDLE", "@senin_hesabin")
